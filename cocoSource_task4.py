...
class imageCaptionModel(nn.Module):
    def __init__(self, config):
        ...
        self.inputlayer = nn.Sequential(
                                        ...
                                        nn.Conv1d(self.number_of_cnn_features,self.hidden_state_sizes,1),
    ...
    def forward(self, cnn_features, xTokens, is_train, current_hidden_state=None):
        ...
        features_transposed = torch.transpose(cnn_features, 1, 2)
        imgfeat_tmp = self.inputlayer(features_transposed)
        imgfeat_pooled = max_pool(imgfeat_tmp)
        imgfeat_permute = imgfeat_pooled.permute(2, 0, 1)
        imgfeat_processed = torch.unbind(imgfeat_permute, dim=0)[0]
        ...
        logits, current_hidden_state_out = self.rnn(xTokens, imgfeat_processed , initial_hidden_state, self.outputlayer, self.Embedding, self.attn, cnn_features, is_train)
...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_state_size, num_rnn_layers, cell_type='GRU'):
        ...
        self.last_layer_state_size = 512+2048
        ...
        for _ in range(self.num_rnn_layers-2):
            input_size_list.append(self.hidden_state_size)
        input_size_list.append(self.last_layer_state_size)
    ...
    def forward(self, xTokens, baseimgfeat, initial_hidden_state, outputLayer, Embedding, attn_layer, cnn_unprocessed_features, is_train=True):
        """
        Args:
            xTokens:        shape [batch_size, truncated_backprop_length]
        ...
        for kk in range(seqLen):
            ...
            for i in range(self.num_rnn_layers):
                if (i == self.num_rnn_layers-1) and (i != 0):
                    attn_weight = attn_layer(updatedstate[-1])
                    weighted_features = cnn_unprocessed_features * attn_weight[:,:, None]
                    weighted_sum_features = torch.sum(weighted_features, dim=1)
                    lvl0input = torch.cat((lvl0input, weighted_sum_features), dim=1)
